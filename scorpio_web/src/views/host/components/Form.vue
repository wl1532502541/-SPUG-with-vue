      ...
      <a-form-item required label="连接地址" style="margin-bottom: 0">
        ...
        <a-form-item style="display: inline-block; width: 40%">
          <a-input
            v-decorator="[
              'ip',
              { rules: [{ required: true }], initialValue: record.ip },
            ]"
            addon-before="@"
            placeholder="主机名/IP"
          >
        ...
        <a-form-item style="display: inline-block; width: 30%">
          <a-input
            v-decorator="[
              'port',
              { rules: [{ required: true }], initialValue: record.port },
            ]"
            addon-before="-p"
            placeholder="端口"
          >
...
export default {
  ...
  methods: {
    ...mapMutations("host", ["fetchRecords","closeForm"]),
    handleSubmit() {
      ...
      http.post("/host/add", formData).then(
        (res) => {
          if (res === "auth fail") {
            if (formData.pkey) {
            ...
            this.loading=false
            // this.$store.state.host.formVisible=false;
            this.fetchRecords();
